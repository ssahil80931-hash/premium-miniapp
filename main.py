...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        c.execute('''INSERT INTO products (title, description, price, image_url, group_link, validity, tags)
                     VALUES (?, ?, ?, ?, ?, ?, ?)''',
                  ("Demo Digital Course", "Full HD Video Course with Lifetime Access", 499,
                   ...
                   "https://t.me/+YourPremiumGroup", "Lifetime", "Premium,VIP"))
...
def get_settings():
    conn = get_db()
    row = conn.execute("SELECT mini_app_name, upi_id, bill_description, total_views FROM settings WHERE id=1").fetchone()
    conn.close()
...
def admin_products():
    ...
    # POST - Add product
    data = request.json
    conn.execute('''INSERT INTO products (title, description, price, image_url, group_link, validity, tags, is_active)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                 (data.get("title"), data.get("description"), float(data.get("price", 0)),
                  data.get("image_url"), data.get("group_link"), data.get("validity", "Lifetime"),
...
def admin_product_detail(pid):
    ...
    # PUT - Update
    data = request.json
    conn.execute('''UPDATE products SET title=?, description=?, price=?, image_url=?, group_link=?,
                    validity=?, tags=?, is_active=? WHERE id=?''',
                 (data.get("title"), data.get("description"), float(data.get("price", 0)),
                  data.get("image_url"), data.get("group_link"), data.get("validity", "Lifetime"),
