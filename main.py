...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS settings (
        ...
        support_telegram TEXT DEFAULT 'admin',
        qr_image_url TEXT DEFAULT '',
        phonepe_link TEXT DEFAULT ''
    )''')
    ...
    migrations = [
        ...
        "ALTER TABLE settings ADD COLUMN support_telegram TEXT DEFAULT 'admin'",
        "ALTER TABLE settings ADD COLUMN qr_image_url TEXT DEFAULT ''",
        "ALTER TABLE settings ADD COLUMN phonepe_link TEXT DEFAULT ''",
        "ALTER TABLE settings ADD COLUMN total_views INTEGER DEFAULT 0",
    ]
    ...
    if c.fetchone()[0] == 0:
        c.execute('''INSERT INTO settings 
            (id, upi_id, mini_app_name, bill_description, admin_password, support_telegram, qr_image_url, phonepe_link) 
            VALUES (1, 'yourupi@paytm', 'Premium Group Hub', 'Digital Course Access - Lifetime', 'baba123', 'admin', '', '')''')
...
def get_settings():
    conn = get_db()
    row = conn.execute("""
        SELECT mini_app_name, upi_id, bill_description, total_views, 
               support_telegram, qr_image_url, phonepe_link 
    ...
    data['support_telegram'] = data.get('support_telegram') or 'admin'
    data['qr_image_url'] = data.get('qr_image_url') or ''
    data['phonepe_link'] = data.get('phonepe_link') or ''
    return jsonify(data)
...
def get_qr(pid):
    ...
    settings = conn.execute("""
        SELECT upi_id, mini_app_name, bill_description, qr_image_url, phonepe_link 
        FROM settings WHERE id=1
    """).fetchone()
    conn.close()
    ...
    upi_id = settings["upi_id"] or "yourupi@paytm"
    app_name = settings["mini_app_name"] or "Premium Group Hub"
    qr_b64 = generate_qr_base64(upi_id, product["price"], app_name)
    upi_link = f"upi://pay?pa={upi_id}&pn={app_name}&am={product['price']}&cu=INR&tn=Premium Access"
    ...
    return jsonify({
        ...
        "bill_description": settings["bill_description"] or "Premium Access Payment",
        "phonepe_link": settings["phonepe_link"] or ""
    })
...
def admin_dashboard():
    ...
    total_views = conn.execute("SELECT COALESCE(total_views,0) FROM settings WHERE id=1").fetchone()[0]
...
def admin_settings():
    ...
    if request.method == 'GET':
        row = conn.execute("""
            SELECT upi_id, mini_app_name, bill_description, admin_password,
                   support_telegram, qr_image_url, phonepe_link 
            FROM settings WHERE id=1
        """).fetchone()
        conn.close()
    ...
    conn.execute('''UPDATE settings SET 
        upi_id=?, mini_app_name=?, bill_description=?, admin_password=?,
        support_telegram=?, qr_image_url=?, phonepe_link=? 
        WHERE id=1''',
        (
            data.get("upi_id"),
            data.get("mini_app_name"),
            data.get("bill_description"),
            data.get("admin_password"),
            (data.get("support_telegram") or "admin").replace("@", ""),
            data.get("qr_image_url") or "",
            data.get("phonepe_link") or ""
        ))
    conn.commit()
    conn.close()
    return jsonify({"success": True})
